...
import { MotionLi } from './components/framer-motion/motion-li'
...
const AdvantageCard = ({
    ...
    icon,
    description,
}: AdvantageCardType & { index: number }) => (
    <MotionLi
...
export default function Advantages() {
    return (
        <>
            <h4 className="min-[780px]:hidden max-w-screen-2xl mx-auto px-10 font-medium text-2xl">
                É aqui onde as coisas acontecem.
            </h4>
            <ul className="max-w-screen-2xl mx-auto px-10 my-7 gap-x-5 gap-y-10 flex-wrap flex justify-between">
                {AdvantageCardList.map((advantage, index) => (
                    <AdvantageCard
                        index={index}
                        key={advantage.title}
                        {...advantage}
